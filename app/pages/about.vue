<template>
  <div class="container mx-auto py-12">
    <h1 class="text-3xl font-bold mb-6">About LEGO QR & Mosaic Builder</h1>
    <p class="text-lg text-gray-700 mb-4">
      This site lets you generate LEGO building instructions for WiFi QR codes and (soon) image mosaics. It is open source and designed for LEGO fans and makers.
    </p>
    <ul class="list-disc list-inside text-gray-600 mb-6">
      <li>WiFi QR code builder: encode your network, get a parts list and instructions</li>
      <li>Mosaic builder: turn any image into a LEGO mosaic (coming soon)</li>
      <li>Optimized for real LEGO parts and colors</li>
      <li>Deployable to GitHub Pages</li>
    </ul>
    <div class="bg-gray-100 border border-gray-300 rounded-lg p-6 text-gray-700">
      <strong>Project by robokozo</strong><br>
      <a href="https://github.com/robokozo/lego-wifi-qr-builder" target="_blank" class="text-blue-600 underline">GitHub Repository</a>
    </div>
  </div>
</template>

<script setup lang="ts">
// About page placeholder
</script>

<style scoped>
.container {
  max-width: 700px;
}
</style>
